...
#Region "Other functions: Delete rename etc."
    ...
    Public Function GetFiles(ByVal dirpath As String, ByVal mask As String) As FTPdirectory
      Dim result As New FTPdirectory()
      mask = mask.Replace(".", "\.").Replace("*", ".+").Replace("?", ".")
      For Each fi As FTPfileInfo In ListDirectoryDetail(dirpath)
        If fi.FileType = FTPfileInfo.DirectoryEntryTypes.File Then
          If mask = "" Then
            result.Add(fi)
          ElseIf Regex.IsMatch(fi.Filename, mask, RegexOptions.IgnoreCase) Then
            result.Add(fi)
          End If
        End If
      Next
    ...
    Public Function GetFiles(ByVal mask As String) As FTPdirectory
      Return GetFiles(Me.CurrentDirectory, mask)
    End Function
...
#Region "FTP Directory class"
  ...
  Public Class FTPdirectory
    ...
    Public Function GetFilesByExt(Optional ByVal ext As String = "") As FTPdirectory
    ...
    Public Function GetFiles(Optional ByVal mask As String = "") As FTPdirectory
      Dim result As New FTPdirectory()
      mask = mask.Replace(".", "\.").Replace("*", ".+").Replace("?", ".")
      For Each fi As FTPfileInfo In Me
        If fi.FileType = FTPfileInfo.DirectoryEntryTypes.File Then
...
'    Dim ftp As New FTPclient("ftp.custom-sw.com", "custom-sw.com", "ftp.58")

'    'download a file (permitting overwrite)
'    ftp.Download("/Data/Montepio/Novetats_Prova/tl005436.dbf", "C:\ftpfile.bin", True)

'    'upload a file
'    ftp.Upload("c:\ftpfile.bin", "/Data/Montepio/Novetats_Prova/upload.exe")

'    'does a file exist?
'    If ftp.FtpFileExists("/Data/Montepio/Novetats_Prova/upload.exe") Then
'      'rename a file
'      ftp.FtpRename("/Data/Montepio/Novetats_Prova/upload.exe", "/Data/Montepio/Novetats_Prova/tl005436.bak")
'      'delete a file
'      ftp.FtpDelete("/Data/Montepio/Novetats_Prova/tl005436.bak")
'    End If

'    'get a simple listing of the root directory /
'    Dim result As List(Of String) = ftp.ListDirectory("/Data/Montepio/Novetats_Prova/")
'    'display in console
...
'    Dim dirList As FTPdirectory = ftp.GetFiles("/Data/Montepio/Novetats_Prova/")

'    'filter out only the files in the list
'    Dim filesOnly As FTPdirectory = dirList.GetFiles("tf*.flg")

'    'download these files
'    For Each file As FTPfileInfo In filesOnly
'      ftp.Download(file, "C:\" & file.Filename, True)
'    Next
