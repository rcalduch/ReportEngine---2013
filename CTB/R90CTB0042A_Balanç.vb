...
Public Class R90CTB0042A_Balanç
    ...
    Public Overrides Sub Execute(workInfo As String)
        ...
        Try
            ...
            rpt.TitolLlistat = CNull(params.GetValue("TitolLlistat")).Replace("CUENTAS ANUALES: BALANCE", "COMPTES ANUALS - BALANÇ")
        ...
        Catch ex As Exception
            ...
            DebugLog(AppData.Debug, Date.Now.ToString("dd/MM/yyyy HH:mm:ss") + " R90CTB0042A_ComptesAnualsBalanç " + ex.Message)
...
Public Class R90CTB0042C_Balanç
...
#End Region
    ...
    Private Function DrawLinBalansSit(ByVal Canvas As System.Drawing.Graphics) As Boolean
        ...
        balanç = lin("Balans").ToString.Trim
        ...
        Try
            If Not String.IsNullOrWhiteSpace(Balanç) Then

                DrawString(Canvas, balanç, fntLineB, Brushes.Black, New RectangleF(curX, CurY, 150, 16), sfNear)
                CurY += deltaLine \ 2

            ElseIf Concepte.StartsWith("TOTAL (") And String.IsNullOrWhiteSpace(titol + subtitol + apartat + subapartat) Then

                CurY += deltaLine \ 2

                ' curX += ColumnWidth(col) + ColumnGap : col += 1 : TotalCol = curX
                ' curX += ColumnWidth(col) + ColumnGap : col += 1 : TotalWidth = ColumnWidth(col) * 2 + ColumnGap
                DrawString(Canvas, concepte, fntLineB, Brushes.Black, New RectangleF(curX, CurY, ColumnWidth(col), 12), sfFar)
                curX += ColumnWidth(col) + ColumnGap : col += 1
                curX += ColumnWidth(col) + ColumnGap : col += 1
                curX += ColumnWidth(col) + ColumnGap : col += 1
                ...
                If Concepte.StartsWith("TOTAL (A + B + C + D)") Then
                    ForceNewPage = True
                End If

            ElseIf Not String.IsNullOrWhiteSpace(titol) Then
            ...
            ElseIf Not String.IsNullOrWhiteSpace(subtitol) Then
                ...
                curX += ColumnWidth(col) + ColumnGap : col += 1
                curX += ColumnWidth(col) + ColumnGap : col += 1
                DrawString(Canvas, fmtValueToStr(lin("sdo_ptd_n"), 2, True), fntLine, Brushes.Black, New RectangleF(curX, CurY, ColumnWidth(col), 12), sfFar)
                curX += ColumnWidth(col) + ColumnGap : col += 1
                DrawString(Canvas, fmtValueToStr(lin("sdo_ptd_n1"), 2, True), fntLine, Brushes.Black, New RectangleF(curX, CurY, ColumnWidth(col), 12), sfFar)
            ...
            ElseIf Not String.IsNullOrWhiteSpace(Compte) Then
                ...
                DrawString(Canvas, compte + "   " + concepte, fntLine, Brushes.Black, New RectangleF(curX + 90, CurY, ColumnWidth(col), 16), sfNear)
                curX += ColumnWidth(col) + ColumnGap : col += 1
                DrawString(Canvas, fmtValueToStr(lin("sdo_cmp_n"), 2, True), fntLine, Brushes.Black, New RectangleF(curX, CurY, ColumnWidth(col), 12), sfFar)
                curX += ColumnWidth(col) + ColumnGap : col += 1
                DrawString(Canvas, fmtValueToStr(lin("sdo_cmp_n1"), 2, True), fntLine, Brushes.Black, New RectangleF(curX, CurY, ColumnWidth(col), 12), sfFar)
    ...
    Private Sub clsImpresFactura_GetDataSource() Handles Me.GetDataSource
        ...
        Try
            ...
            lin = New csTableReader(dbaRpt.get_comptes_anuals_balans)
